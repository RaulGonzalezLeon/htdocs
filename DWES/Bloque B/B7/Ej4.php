...
function redimensionar_gd($ruta_origen, $ruta_nueva, $maximo_ancho, $maximo_largo){
    $datos_imagen = getimagesize($ruta_origen);
    $ancho_origen = $datos_imagen[0];
    $alto_origen = $datos_imagen[1];
    $tipo_medio = $datos_imagen['mime'];
    $nuevo_ancho = $maximo_ancho;
    $nuevo_alto = $maximo_largo;
    $ratio_origen = $ancho_origen / $alto_origen;

    if ($ancho_origen > $alto_origen){
        $nuevo_alto = $nuevo_ancho / $ratio_origen;
    } else {
        $nuevo_ancho = $nuevo_alto * $ratio_origen;
    }
    ...
    $nuevo = imagecreatetruecolor($nuevo_ancho, $nuevo_alto);
    imagecopyresampled($nuevo, $origen, 0, 0, 0, 0, $nuevo_ancho, $nuevo_alto, $ancho_origen, $alto_origen);
    
    switch($tipo_medio) {
        case 'image/gif': imagegif($nuevo, $ruta_nueva);  break;
        case 'image/jpeg': imagejpeg($nuevo, $ruta_nueva); break;
        case 'image/png': imagepng($nuevo, $ruta_nueva);  break;
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($_FILES['image']['error'] == 0) {
        ...
        if (!$error) {
            ...
            $resized = redimensionar_gd($destino, $thumbpath, 200, 200);
        }
    }
...
<?= $mensaje; ?>
<form method="POST" action="Ej4.php" enctype="multipart/form-data">
