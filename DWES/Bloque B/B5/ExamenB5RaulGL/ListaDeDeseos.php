...
class ListaDeDeseos
{
    ...
    public function limpiarPalabrasProhibidas(string $texto): string
    {
        foreach ($this->palabrasProhibidas as $palabra) {
            ...
            while (preg_match($regex, $texto)) {
                $texto = preg_replace($regex, str_repeat('*', strlen($palabra)), $texto, 1);
            }
        }
    ...
    public function agregarCarta(string $texto)
    {
        ...
        $textoFiltrado = $this->limpiarPalabrasProhibidas($textoLimpio);
        $this->cartas[] = $textoFiltrado;
    }
    ...
    {
        $claveAleatoria = array_rand($this->posiblesRegalos);
        array_push($this->regalosDisponibles, $claveAleatoria);
    }
    ...
    foreach ($repartoDeRegalos as $index => $regalos) {
        $deseoCasa = $this->cartas[$index];
        $cumpleDeseo = false;
        foreach ($regalos as $regalo) {
            if (strpos($deseoCasa, $regalo) !== false) {
                $cumpleDeseo = true;
                break;
            }
        }
        $resultado[] = $cumpleDeseo;
    }
    return $resultado;
    }
}
