...
class ListaDeDeseos
{
     ...
     */
    public function __construct(array $palabrasProhibidas = [], array $posiblesRegalos = ["bicicleta" => 299.99, "patinete" => 499.90, "videoconsola" => 599.99])
    {
        $this->cartas = [];
    ...
    {
        // Comprueba si el nombre del regalo es "carbón".
        if ($nombreRegalo === "carbón") {
            // Si el regalo es "carbón", se devuelve un precio de 0.
            return 0;
        } else {
            // En caso contrario, devuelve el precio asociado al regalo 
    ...
    {
        // Elimina espacios al inicio y al final del texto y actualiza la variable.
        $texto = trim($texto);
    
        // Convierte el texto a minúsculas y actualiza la variable.
        $texto = strtolower($texto);
    
        // Retorna el texto "limpiado".
        return $texto;
    }
    ...
    {
        // Variable que se usará para la sustitución de las palabras prohibidas.
        $sustitucion = "";
    
        // Recorre cada una de las palabras prohibidas almacenadas en el arreglo.
        foreach ($this->palabrasProhibidas as $palabrasProhibidas) {
            // Verifica si la palabra prohibida está presente en el texto usando una expresión regular.
            $regex = preg_match("/$palabrasProhibidas/", $texto);

            // Mientras se encuentre la palabra prohibida en el texto, se sigue reemplazando.
            while ($regex) {
                // Reemplaza la palabra prohibida con asteriscos (tantos como la longitud de la palabra).
                $texto = preg_replace("/$palabrasProhibidas/", str_repeat("*", strlen($palabrasProhibidas)), $texto);
                // Vuelve a comprobar si aún existe la palabra prohibida en el texto.
                $regex = preg_match("/$palabrasProhibidas/", $texto);
            }
        }
        return $texto;
    ...
    {
        // Limpia el texto de la carta utilizando la función limpiarTexto.
        $texto = $this->limpiarTexto($texto);

        // Elimina palabras prohibidas del texto utilizando la función limpiarPalabrasProhibidas.
        $texto = $this->limpiarPalabrasProhibidas($texto);

        // Agrega el texto limpio al arreglo de cartas.
        array_push($this->cartas, $texto);
    }
    ...
    {
        // Llama a la función producirNuevoRegalo para asegurarse de que hay al menos un regalo disponible.
        $this->producirNuevoRegalo();

        // Extrae y devuelve el primer regalo disponible de la lista.
    ...
    {
        // Selecciona aleatoriamente una clave del arreglo de posibles regalos.
        $producirRegalo = array_rand($this->posiblesRegalos);

        // Agrega el regalo seleccionado a la lista de regalos disponibles.
        array_push($this->regalosDisponibles, $producirRegalo);
    }
     ...
     */
// Función que analiza si el reparto de regalos cumple con las cartas recibidas.
    public function analisisReparto(array $repartoDeRegalos): array
    {
        $resultado = []; 

        // Bucle principal que recorre cada conjunto de regalos repartidos.
        for ($i = 0; $i < count($repartoDeRegalos); $i++) {
            $repartoCumplido = false; 

            // Bucle interno que recorre los regalos de la carta actual.
            for ($j = 0; $j < count($repartoDeRegalos[$i]); $j++) {
                $regalos = $repartoDeRegalos[$i][$j];

                // Se verifica si el regalo está presente en el contenido de la carta correspondiente.
                if (preg_match("/$regalos/", $this->cartas[$i])) {
                    $repartoCumplido = true;
                    break;
                }
            }

            // Se almacena el resultado (true o false) en el arreglo final.
            $resultado[] = $repartoCumplido;
        }

        // Devuelve un arreglo con los resultados del análisis para cada carta.
        return $resultado;
    }
}
