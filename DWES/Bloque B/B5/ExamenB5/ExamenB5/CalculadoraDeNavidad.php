...
class CalculadoraDeNavidad
{
    ...
    {
        // Inicializa un array para almacenar el número de regalos por casa
        $regalos = [];

        // Distribuye 3 regalos a cada casa inicialmente
        for ($i = 0; $i < $totalCasas; $i++) {
            $regalos[$i] = 3;  
            $totalRegalos -= 3;
        }

        // Distribuye el resto de regalos de manera equitativa entre las casas
        $j = 0;
        ...
        // Retorna el array con la distribución de regalos por casa
        return $regalos;
    }
    ...
    {
        // Inicializa un array para almacenar los regalos asignados a cada casa
        $regalosPorCasa = [];
        
        // Itera sobre cada casa en la asignación de regalos
        for ($i = 0; $i < count($asignacionDeRegalos); $i++) {
            // Inicializa un array vacío para almacenar los regalos de la casa actual
            $regalosPorCasa[$i] = [];
            
            // Obtiene las cartas de deseos usando el método `obtenerCartas` de la clase `ListaDeDeseos`
            $carta = $deseos->obtenerCartas();
            
            // Verifica si la carta de la casa actual contiene dos o más caracteres repetidos (patrón `**` o más)
            if (preg_match('/\*{2,}/', $carta[$i])) {
                // Si se cumple la condición, asigna carbón como regalo
                $regalosPorCasa[$i][] = "carbón";
            } else {
                // Si no se cumple la condición, asigna los regalos correspondientes
                for ($j = 0; $j < $asignacionDeRegalos[$i]; $j++) {
                    // Obtiene un regalo aleatorio del catálogo de deseos
                    $regalosPorCasa[$i][] = $deseos->obtenerRegalo();
                }
            }
        }
        
        // Devuelve el array con los regalos asignados por casa
    ...
    {
        // Inicializa la variable para el coste total.
        $coste = 0;
        
        // Itera sobre cada regalo en la lista proporcionada.
        foreach ($regalos as $regalo) {
            // Obtiene el precio del regalo usando la función `obtenerPrecioRegalo` de la clase `ListaDeDeseos`.
            $precio = $listaDeseos->obtenerPrecioRegalo($regalo);
            
            // Suma el precio del regalo al coste total.
            $coste += $precio;
        }
        
        // Devuelve el coste total calculado.
        return $coste;
    }
...
{
    //Número total de regalos repartidos
    $totalRegalos = 0;
    foreach ($repartoRegalos as $casa) {
        $totalRegalos += count($casa); // Contar los regalos de cada casa
    }
    ...
    //Determinar si la Navidad fue feliz
    $navidadFeliz = $porcentajeAciertos >= 50;

    // 6. Mostrar las estadísticas en formato HTML
    echo "<h2>Estadísticas del Reparto de Regalos</h2>";
    echo "<ul>";
    echo "<li>Número total de regalos repartidos: <strong>{$totalRegalos}</strong></li>";
